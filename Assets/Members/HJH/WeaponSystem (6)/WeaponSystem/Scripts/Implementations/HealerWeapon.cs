...
namespace RobotWeapons
{
    public class HealerWeapon : WeaponBase
    {
        ...
        private void FireHealShot()
        {
            ...
            Quaternion muzzleRot = AimUtility.GetConvergedMuzzleRotation(owner, owner.AimOrigin.forward, data.healShotAimRange);

            GameObject shot = GameObject.Instantiate(data.healShotPrefab, owner.MuzzleOrigin.position, muzzleRot);
        ...
        private void ThrowGrenade()
        {
            ...
            GameObject g = GameObject.Instantiate(data.grenadePrefab, owner.MuzzleOrigin.position, Quaternion.identity);

            float rad = data.throwUpwardAngle * Mathf.Deg2Rad;
            Vector3 dir = (owner.AimOrigin.forward * Mathf.Cos(rad) + Vector3.up * Mathf.Sin(rad)).normalized;
